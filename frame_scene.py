    ...
    with tqdm(train_dataloader, unit='batch') as tepoch:
        for batch_data in tepoch:
            ...
            for item_data in batch_data:        
                ...
                # # ===========> 2. optimize shot and clustering
                ...
                if len(sampled_scene_coups) == 0:
                    ...
                    loss_batch_scene_list.append(loss_sample_shot/ len(sampled_scene_coups))
                    score_batch_scene_list.append(score_sample_shot/ len(sampled_scene_coups))
            ...
            score_step_shot = sum(score_batch_scene_list) / len(score_batch_scene_list)
            loss_step_shot = sum(loss_batch_scene_list) / len(loss_batch_scene_list)
            ...
            score_epoch_scene_list.append(float(score_step_shot))
            loss_epoch_scene_list.append(float(loss_step_shot))
            # wandb.log({'train loss':loss_step.item(), 'train score':score_step})
            tepoch.set_postfix(loss=sum(loss_epoch_list)/len(loss_epoch_list), loss_cluster=sum(loss_epoch_cluster_list)/len(loss_epoch_cluster_list), score=sum(score_epoch_list)/len(score_epoch_list), shot_score=sum(score_epoch_scene_list)/len(score_epoch_scene_list), lzr=lr_scheduler.get_lr()[0])
            ...
            for batch_data in tepoch:
                ...
                for item_data in batch_data:        
                    ...
                    if n_scene == 1:
                        ...
                        # cluster_output, _ = kmeans(X=hidden_states, num_clusters=n_scene, distance='cosine', device=device, iter_limit=1000, tqdm_flag=False)
                        # cluster_output, _ = kmeans(X=hidden_states, num_clusters=n_scene, distance='soft_dtw', device=device, iter_limit=20, tqdm_flag=False)
        ...
        for batch_data in tepoch:
            ...
            for item_data in batch_data:        
                ...
                if n_scene == 1:
                    ...
                    cluster_output, _ = kmeans(X=hidden_states, num_clusters=n_scene, distance='cosine', device=device, iter_limit=1000, tqdm_flag=False)
                    # cluster_output, _ = kmeans(X=hidden_states, num_clusters=n_scene, distance='euclidean', device=device, iter_limit=1000, tqdm_flag=False)
                    # cluster_output, _ = kmeans(X=hidden_states, num_clusters=n_scene, distance='soft_dtw', device=device, iter_limit=20, tqdm_flag=False)
